  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
      <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Dodaj typ zwierzęcia</h3>
      ...
      <div class="p-6">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nazwa typu</label>
        <input
          v-model="name"
          type="text"
...
const handleSave = async () => {
  ...
  try {
    ...
    saveError.value = err?.response?.data?.message ?? 'Nie udało się dodać typu.';
  } finally {

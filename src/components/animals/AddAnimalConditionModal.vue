  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
      <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Add animal condition</h3>
      ...
      <div class="p-6">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Condition name</label>
        <input
          v-model="name"
          type="text"
          ...
          placeholder="e.g. Excellent, Critical..."
      ...
      <div class="p-6 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-700">
        ...
        <button :disabled="isSaving || !name.trim()" @click="handleSave" class="bg-[#2d6a4f] hover:bg-[#1a3b22] disabled:opacity-60 disabled:cursor-not-allowed text-white px-5 py-2 rounded-xl font-medium transition-colors shadow-sm flex items-center gap-2 min-w-[160px] justify-center">
...
const handleSave = async () => {
  ...
  try {
    const saved = await animalService.createCondition(name.value.trim());
    emit('save', saved);
    name.value = '';
  } catch (err) {
    saveError.value = parseApiError(err, 'Failed to add condition.');
  } finally {

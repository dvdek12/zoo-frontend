<template>
  ...
  >
    ...
    <template #cell-name="{ row }">
      <div class="flex items-center gap-2">
        <div class="w-6 h-6 rounded-md bg-[#f0f9f4] dark:bg-[#132a1e] text-[#2d6a4f] dark:text-green-400 flex items-center justify-center font-bold text-xs">
          {{ (row.condition ?? row.name ?? '?').charAt(0).toUpperCase() }}
        </div>
        <span class="font-medium text-gray-900 dark:text-white">{{ row.condition ?? row.name }}</span>
      </div>
    </template>
    ...
    <!-- Empty state -->
    <template #empty>
      <p class="text-sm text-gray-400 py-4">No animal conditions.</p>
    </template>
  </DataTable>
</template>

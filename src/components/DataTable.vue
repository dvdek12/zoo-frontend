    ...
    <Transition name="fade">
      <div v-if="rows.length === 0" class="flex-1 flex flex-col items-center justify-center select-none overflow-hidden py-12">
        <slot name="empty">
          ...
          <p class="text-lg font-semibold text-gray-400 dark:text-gray-500 mb-1">Brak danych</p>
          <p class="text-sm text-gray-300 dark:text-gray-600 text-center max-w-xs">Dodaj pierwszy rekord używając przycisku powyżej.</p>
        </slot>
      </div>
    </Transition>
  </div>
